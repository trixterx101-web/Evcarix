...
TOPIC_STYLES = {
    "electric_vehicle": {
        "bg":      (0,0,0),    "left_bg":  (10,0,0),
        "right_bg":(0,10,24),  "accent1":  (255,34,0),
        "accent2": (0,212,255),"label":    "EV DATA",   "icon": "⚡",
        "stats": [("+300%","EV SALES GROWTH",(255,34,0)),
                  ("$0",   "GAS CAR FUTURE VALUE",(0,212,255)),
                  ("500MI","NEW EV RANGE RECORD",(255,204,0))],
    },
    "battery_tech": {
        "bg":      (0,0,0),    "left_bg":  (10,5,0),
        "right_bg":(0,10,5),   "accent1":  (255,107,0),
        "accent2": (0,255,136),"label":    "BATTERY",   "icon": "🔋",
        "stats": [("1M KM", "LFP LIFESPAN",         (255,107,0)),
                  ("10 MIN","FUTURE CHARGE TIME",    (0,212,255)),
                  ("-45%",  "WINTER RANGE LOSS",     (0,255,136))],
    },
    "artificial_intelligence": {
        "bg":      (0,0,0),    "left_bg":  (5,0,10),
        "right_bg":(0,0,10),   "accent1":  (139,0,255),
        "accent2": (0,212,255),"label":    "AI TECH",   "icon": "🤖",
        "stats": [("10x",  "SPEED VS HUMAN",         (139,0,255)),
                  ("2030", "AGI PREDICTION",          (0,212,255)),
                  ("$1T",  "AI MARKET SIZE",          (255,204,0))],
    },
    "robotics": {
        "bg":      (0,0,0),    "left_bg":  (0,10,0),
        "right_bg":(0,5,5),    "accent1":  (0,255,136),
        "accent2": (255,255,255),"label":  "ROBOTICS",  "icon": "🦾",
        "stats": [("40%",  "JOBS AUTOMATED",          (0,255,136)),
                  ("$1.5T","ROBOTICS MARKET",          (0,212,255)),
                  ("24/7", "ROBOT UPTIME",             (255,204,0))],
    },
    "future_tech": {
        "bg":      (0,0,0),    "left_bg":  (5,0,10),
        "right_bg":(10,0,5),   "accent1":  (255,0,255),
        "accent2": (255,204,0),"label":    "FUTURE",    "icon": "🚀",
        "stats": [("+300%","EV SALES GROWTH",          (0,212,255)),
    ...
    "default": {
        "bg":      (0,0,0),    "left_bg":  (10,0,0),
        "right_bg":(0,10,24),  "accent1":  (255,34,0),
        "accent2": (0,212,255),"label":    "EV TECH",   "icon": "⚡",
        "stats": [("+300%","EV SALES GROWTH",           (255,34,0)),
                  ("$0",   "GAS CAR FUTURE VALUE",      (0,212,255)),
                  ("500MI","NEW EV RANGE RECORD",        (255,204,0))],
    },
...
def _split_title(title: str) -> list[str]:
    ...
    while len(lines) < 3:
        lines.append("")
    return [_safe(l) for l in lines[:3]]
...
def _corners(draw, a1, a2):
    W,H = 1280,720; s=50; t=5; m=15; mb=83
...
def _layout_split(W, H, lines, st):
    ...
    for x in range(W//2+80):
        t = x/(W//2+80)
        c = _mix(st["left_bg"], st["bg"], t)
        draw.line([(x,0),(x,H)], fill=c)
    # Right gradient
    for x in range(W//2-80, W):
        t = (x-(W//2-80))/(W-(W//2-80))
        c = _mix(st["bg"], st["right_bg"], t)
        draw.line([(x,0),(x,H)], fill=c)
    # Glows
    ...
    for i in range(6):
        x = W//2-30+i*10
        draw.line([(x,0),(x+100,H)], fill=(*a1,40) if i<3 else (*a2,40), width=2)
    # Left accent bar
    ...
    draw.rectangle([x_l-10,strike_y-6,x_l+int(draw.textlength(lines[1],font=f2))+10,strike_y+6], fill=a1)
    # Sub
    if lines[2]:
...
def _layout_versus(W, H, lines, st):
    ...
    sf = _fnt(22)
    draw.rectangle([14,14,int(draw.textlength("🔥 SHOCKING",font=sf))+50,62], fill=(255,90,0))
    draw.text((22,18), "🔥 SHOCKING", font=sf, fill=(255,255,255))
    draw.text((int(draw.textlength("🔥 SHOCKING",font=sf))+60,18),
              "GAS CAR OWNERS MUST SEE THIS", font=_fnt(20,False), fill=(200,200,200))
    # Left content
    ...
    for y in range(82, H-74):
        t = abs(y-(H//2))/(H//2)
        alpha = int(200*(1-t))
        draw.point((vx,y), fill=(*a1[:3],))
    # Right: stat cards
    cx,cy,cw = vx+18, 90, W-vx-36
    for val, lbl, col in st.get("stats", []):
        cr,cg,cb = col
        ...
        draw.text((cx+14,cy+58), lbl, font=_fnt(18,False), fill=(165,180,195))
        cy += 104
    _brand_bar(draw,W,H,a1,a2,st["label"])
    _corners(draw,a1,a2)
    return img
...
def _layout_data(W, H, lines, st):
    ...
    bx, by_s = 36, H-148
    bw_s = (W-72-40)//3
    for i,(val,lbl,col) in enumerate(st.get("stats",[])):
        x  = bx + i*(bw_s+20)
        cr,cg,cb = col
        ...
        draw.text((x+12,by_s+48), lbl[:14], font=_fnt(14,False), fill=(150,165,178))
    _brand_bar(draw,W,H,a1,a2,st["label"])
    _corners(draw,a1,a2)
    return img
...
class ThumbnailGenerator:
    ...
    def create(self, title: str, topic: str = "default", stat: str = "",
               category: str = "default", output_path: str = "",
               is_short: bool = False) -> str:
        # Short video → skip graphic thumbnail
        if is_short:
            if output_path:
                os.makedirs(os.path.dirname(output_path) if os.path.dirname(output_path) else ".", exist_ok=True)
                Image.new("RGB",(1080,1920),(0,0,0)).save(output_path,"JPEG",quality=60)
        ...
        try:
            ...
            logger.info(f"[Thumbnail] ✅ {layout} — {size}KB → {output_path}")
            return output_path
    ...
    def upload_thumbnail(self, video_id: str, thumbnail_path: str) -> bool:
        try:
            ...
            logger.info(f"[Thumbnail] ✅ YouTube'a yüklendi: {video_id}")
            return True
        except Exception as e:
